---
title: Full release runbook
description: Preserved v0.8.0 operating notes, validation evidence, and deployment status.
---

These are the detailed package notes retained when the Fumadocs site was added. Start with the focused skill pages for everyday use.
...
| --- | --- | --- |
| [Autopsy](https://github.com/New1Direction/MSK/blob/main/skills/autopsy/SKILL.md) | Where did launch supply go, and what explains early wallet activity? | EVM ERC-20 transfers; broader investigation uses available evidence. |
| [Exit Doctor](https://github.com/New1Direction/MSK/blob/main/skills/exit-doctor/SKILL.md) | What does exiting this position size look like after impact and costs? | One canonical 30-bps V2 pool. |
| [Meme Scout](https://github.com/New1Direction/MSK/blob/main/skills/meme-scout/SKILL.md) | Is a narrative spreading through original participation or amplification? | Retained social observations; no bundled X/Grok client. |
| [Ignition](https://github.com/New1Direction/MSK/blob/main/skills/ignition/SKILL.md) | Is early buying participation accelerating? | Normalized swap observations and a signal/outcome journal. |
| [Scout Network](https://github.com/New1Direction/MSK/blob/main/skills/scout-network/SKILL.md) | Were a discovery wallet's historical finds realistically followable? | Supplied point-in-time cohorts and follower execution evidence. |
| [Second Wind](https://github.com/New1Direction/MSK/blob/main/skills/second-wind/SKILL.md) | Is a dormant token attracting new participation? | Supplied historical windows and coverage. |
| [LP Edge](https://github.com/New1Direction/MSK/blob/main/skills/lp-edge/SKILL.md) | Do fees compensate for inventory changes and costs versus holding? | Canonical V3 accounting, range scenarios and read-only wallet calls. |
| [Undertow](https://github.com/New1Direction/MSK/blob/main/skills/undertow/SKILL.md) | Is a Robinhood meme gaining against its stock quote, and where is observed capital rotating? | Chain4663 attribution, supplied attributed flows, bounded canonical V4 evidence, current stock-reference capture. |
| [PULSE](https://github.com/New1Direction/MSK/blob/main/skills/pulse/SKILL.md) | Which source delivers usable Robinhood data first, and how do we operate our own data path? | Multi-source WS/HTTP collection, RPC Race, provisional and reconciled V4 state, private Nitro planning and probes. |
| [HOOK LAB](https://github.com/New1Direction/MSK/blob/main/skills/hook-lab/SKILL.md) | What does this Robinhood V4 hook do, and what evidence supports this exact wallet call? | Exact deployment identity, Pons V2 source-derived fees, pinned calls/traces, isolated Anvil fork balances, evidence consistency and drift checks. |
| [PRESSURE](https://github.com/New1Direction/MSK/blob/main/skills/pressure/SKILL.md) | Is stock-token supply changing, where are balances moving, and what do measured liquidity observations support? | Bounded full-receipt collection, raw supply/multiplier reconciliation, custody observations, retained trade-size comparisons and prospective outcome journals. |
| [WATCHTOWER](https://github.com/New1Direction/MSK/blob/main/skills/watchtower/SKILL.md) | Can we retain every included Robinhood transaction promptly and verify the reported monitored interval has no gaps? | Full RPC blocks and receipts, fresh-tail priority, durable recovery, research workers and source latency comparisons. |
| [CATALYST](https://github.com/New1Direction/MSK/blob/main/skills/catalyst/SKILL.md) | Which newly disclosed company events relate to these exact Robinhood assets and pools? | Bounded SEC submissions/document collection, structured Form 4 parsing, durable evidence and retained pool-response analysis. |
| [Night Desk](https://github.com/New1Direction/MSK/blob/main/skills/night-desk/SKILL.md) | How do stock reference value, onchain price and position-size exit evidence differ? | Robinhood REST references, multiplier accounting, retained exact-asset prices and wallet/size/route-specific quote or simulation evidence. |
| [Agent Arena](https://github.com/New1Direction/MSK/blob/main/skills/agent-arena/SKILL.md) | What did the agent earn after external funding, costs and open inventory? | Supplied portfolio accounting, exact bracketed TWR, observed drawdowns and append-only SQLite evidence journals. |
| [CIRCUIT](https://github.com/New1Direction/MSK/blob/main/skills/circuit/SKILL.md) | Can this exact V4 route settle, what returns to the wallet, and how do sampled sizes compare after estimated costs? | Pinned Universal Router 2.1.1 calldata, bounded path/cycle enumeration, Permit2 preflight, isolated fork balances and conditional Nitro cost estimates. |
...
The example is synthetic. See CIRCUIT's [instructions](https://github.com/New1Direction/MSK/blob/main/skills/circuit/SKILL.md), [interfaces](https://github.com/New1Direction/MSK/blob/main/skills/circuit/references/interfaces.md) and [EVM validation](https://github.com/New1Direction/MSK/blob/main/skills/circuit/references/evm-validation.md) for actual-wallet inputs and reproducible checks. The retained EVM evidence covers open routes, negative/positive spread cycles, minimum and allowance failures, a synthetic fee hook, and native/ERC20 partial-fill refunds. It establishes local protocol behavior with zero source writes, not an actual Robinhood opportunity.
...
See [WATCHTOWER operations](https://github.com/New1Direction/MSK/blob/main/skills/watchtower/references/operations.md) and [validation](https://github.com/New1Direction/MSK/blob/main/skills/watchtower/references/validation.md) for exact scope and reproducible commands.
...
Preserved license notices apply. See [THIRD_PARTY_NOTICES.md](https://github.com/New1Direction/MSK/blob/main/THIRD_PARTY_NOTICES.md); the collection has no blanket MIT or proprietary license declaration.
