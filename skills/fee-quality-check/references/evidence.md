...
Input mode fee-quality-check requires envelope and swaps array with id (canonical blockHash:txHash:logIndex), actor string/null, volume decimal string, lp_fee decimal string/null. Upstream must resolve economic actors or leave null; using the router for everyone creates false concentration. Rows have a common period/pair/denomination and are deduplicated by id; conflicting duplicate IDs are errors. The helper reports known-actor volume coverage, top known actor share of all volume, HHI conditional on known actors, total fees when complete and duplicates removed. It does not infer wallet clusters, automation, wash trading or causality.
