...
Input mode lp-flow-decay uses the same envelope and adjacent same-duration windows as Fee Migration Watch. Each window has exactly one pool row with the same id; volume and lp_fees use the same units. Null fees mean unknown. The helper returns rate changes only, no trigger threshold, half-life, position fee share or forecast. Upstream position-aware analysis requires actual range and fee-growth evidence.
